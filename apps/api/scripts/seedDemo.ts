/**
 * Demo-world seeding, callable from both the one-shot seed script and the
 * container init (first boot only). DROPS and recreates the target database.
 */
...
export async function seedDemo(adminUrl: string, appDb: string): Promise<Record<string, unknown>> {
  const admin = new pg.Client({connectionString: adminUrl});
  await admin.connect();
  await admin.query(`DROP DATABASE IF EXISTS ${appDb} (FORCE)`);
  ...
  await pool.end();
  return counts.rows[0] as Record<string, unknown>;
}
